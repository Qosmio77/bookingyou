...
PAIRS = {
    'zh-HK': [
        ...
        ('收舖後未覆，客人已經搵第二間', '客人想即時知道有冇位，等到第二日先覆往往太遲。', '任何時間都睇到可預約時段', '客人可隨時查看時段並提交申請；網頁預約要商戶確認先成立。'),
        ...
        ('旺時想先審批，淡時又想即時確認', '單一確認方式太死板，未能配合每日不同人手和工作量。', '每個時段獨立設定確認方式', 'App 預約可按時段設定即時確認或商戶審批；網頁預約一律要商戶確認先成立。'),
    ...
    'en': [
        ...
        ('After-hours enquiries go to another business', 'Customers want to know what is available now, not wait until the next morning.', 'Show available times at any hour', 'Customers can view slots and submit a request at any time. Web bookings require merchant confirmation.'),
        ...
        ('Busy slots need approval; quiet slots do not', 'One confirmation rule cannot match changing staffing and workload.', 'Choose confirmation rules by time slot', 'In-app bookings follow the time slot’s instant-confirmation or merchant-approval setting. Web bookings always require merchant confirmation.'),
    ...
    'zh-CN': [
        ...
        ('营业后未及时回复，顾客已选择别家', '顾客想马上知道有没有空位，等到第二天往往太迟。', '随时查看可预约时段', '顾客可随时查看时段并提交申请；网页预约需商户确认后才成立。'),
        ...
        ('忙时想先审批，闲时想即时确认', '单一确认方式太死板，无法配合每天不同的人手和工作量。', '每个时段单独设置确认方式', 'App 预约按时段设置即时确认或商户审核；网页预约均需商户确认后才成立。'),
    ...
    'ja': [
        ...
        ('営業時間外の問い合わせを他店に取られる', 'お客様は翌朝の返信ではなく、今すぐ空き状況を知りたいものです。', 'いつでも空き時間を表示', 'いつでも空き時間を確認して予約を申し込めます。ウェブ予約は店舗の確認後に確定します。'),
        ...
        ('繁忙時は承認制、閑散時は即時確定にしたい', '一律の確定方法では、日ごとの人員や仕事量に対応できません。', '時間枠ごとに確定方法を設定', 'アプリ内予約は時間帯ごとの即時確定・店舗承認の設定に従います。ウェブ予約は必ず店舗の確認後に確定します。'),
    ...
    'ko': [
        ...
        ('영업시간 이후 문의 고객이 다른 곳으로 간다', '고객은 다음 날 답변보다 지금 가능한 시간을 알고 싶어 합니다.', '언제든 예약 가능한 시간 표시', '언제든 빈 시간을 확인하고 예약을 신청할 수 있습니다. 웹 예약은 매장 확인이 필요합니다.'),
        ...
        ('바쁜 시간은 승인, 한가한 시간은 즉시 확정하고 싶다', '하나의 확정 방식으로는 날마다 다른 인력과 업무량에 대응하기 어렵습니다.', '시간대별 확정 방식 설정', '앱 예약은 시간대별 즉시 확정 또는 매장 승인 설정을 따릅니다. 웹 예약은 항상 매장 확인 후 확정됩니다.'),
    ...
    'ms': [
        ...
        ('Pertanyaan selepas waktu operasi pergi kepada pesaing', 'Pelanggan mahu tahu masa yang tersedia sekarang, bukan menunggu hingga esok.', 'Paparkan masa tersedia pada bila-bila masa', 'Pelanggan boleh melihat slot dan menghantar permintaan pada bila-bila masa. Tempahan web memerlukan pengesahan peniaga.'),
        ...
        ('Slot sibuk perlu kelulusan, slot lengang tidak', 'Satu peraturan pengesahan tidak sesuai dengan tenaga kerja dan beban yang berubah.', 'Pilih cara pengesahan bagi setiap slot', 'Tempahan dalam aplikasi mengikut tetapan slot: pengesahan segera atau kelulusan peniaga. Tempahan web sentiasa memerlukan pengesahan peniaga.'),
    ...
    'th': [
        ...
        ('ตอบหลังเวลาร้านไม่ทัน ลูกค้าไปเลือกร้านอื่น', 'ลูกค้าอยากรู้เวลาว่างทันที ไม่อยากรอคำตอบถึงวันถัดไป', 'แสดงเวลาที่จองได้ทุกเมื่อ', 'ลูกค้าดูเวลาว่างและส่งคำขอได้ตลอดเวลา การจองผ่านเว็บต้องรอร้านยืนยัน'),
        ...
        ('ช่วงยุ่งอยากอนุมัติก่อน ช่วงว่างอยากยืนยันทันที', 'วิธียืนยันแบบเดียวไม่เหมาะกับจำนวนพนักงานและงานที่เปลี่ยนในแต่ละวัน', 'กำหนดวิธียืนยันแยกตามช่วงเวลา', 'การจองในแอปจะยืนยันอัตโนมัติหรือรอร้านอนุมัติตามการตั้งค่าแต่ละช่วงเวลา ส่วนการจองผ่านเว็บต้องรอร้านยืนยันทุกครั้ง'),
    ...
    'vi': [
        ...
        ('Khách hỏi ngoài giờ rồi chọn nơi khác', 'Khách muốn biết giờ trống ngay, thay vì chờ đến sáng hôm sau.', 'Hiển thị giờ trống bất cứ lúc nào', 'Khách có thể xem giờ trống và gửi yêu cầu bất cứ lúc nào. Đặt lịch trên web cần cửa hàng xác nhận.'),
        ...
        ('Giờ cao điểm cần duyệt, giờ vắng muốn xác nhận ngay', 'Một quy tắc xác nhận không phù hợp với nhân sự và khối lượng công việc thay đổi.', 'Chọn cách xác nhận theo từng khung giờ', 'Đặt lịch trong ứng dụng tuân theo cài đặt xác nhận ngay hoặc chờ cửa hàng duyệt của từng khung giờ. Đặt lịch trên web luôn cần cửa hàng xác nhận.'),
